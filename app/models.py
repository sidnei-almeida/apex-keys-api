...
class Raffle(Base):
    ...
    featured_tier: Mapped[str | None] = mapped_column(
        String(20),
        nullable=True,
...
class Transaction(Base):
    ...
    payment_hold_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), nullable=True, index=True)
    # JSON: { raffle_id, raffle_title, ticket_numbers } — auditoria se a reserva for cancelada
    raffle_checkout_snapshot: Mapped[dict | None] = mapped_column(JSON, nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
